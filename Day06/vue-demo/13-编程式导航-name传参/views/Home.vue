    ...
    <div class="home">
        ...
        <div class="hot-link">
            ...
            <!-- 
                ...
                动态路由传参:动态路由参数
                1. 在 router/index.js 配置动态路由参数。/search/:参数名
                2. 传递：/search/参数值
                3. 目标组件中接收参数：$route.params.参数名
             -->
            <router-link to="/search/哈哈111">哈哈111</router-link>
            <router-link to="/search/喜喜222">喜喜222</router-link>
            <router-link to="/search/哟哟333">哟哟333</router-link>
        </div>
    </div>
</template>
<script>
export default{
    ...
    methods:{
        toSearch(){
            // 编程式导航：使用 js 代码 进行跳转
            // 跳转方式：1. 使用 path（路径）  2. 使用 name （路由名称）
            // 1. 使用 path （路径）
            ...
            // 2. 使用 name （路由名称）
            // 先去 index.js 中配置 路由的 name
            // this.$router.push({ name:'路由名' })
            // this.$router.push({
            //     name:'search_n'
            // })
            // 2.1 name + query 传参
            // 语法 和 path + query 传参一样，只是把 path 换成 name
            // this.$router.push({
            //     name : 'search_n',
            //     query:{
            //         words : this.words
